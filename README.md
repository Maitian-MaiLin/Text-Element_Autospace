...
* Some common delimiters and surrounding spaces: `•`, `·`, `|`
* U+0000-U+001F: Control character (e.g. new line)
...
* `[Icon]` represents a template like `<span class="icon space-around">[[File:Icon.png]]</span>`
* `_` denotes the spacing element `<span class="text-element-space"> </span>` as sub-element of `[Icon]` generated by this extension.
...
| `text（text） • text` | `text（text）• text` | Remove space between `）` and `•`
| `[icon]    text` | *stay the same* | Only remove the 'spaces' that have exactly one space
