...
在语言属性为 `ja` `zh` `yue` `wuu` `nan` 及其变体的 `.space-around` 元素内侧插入 `.text-element-space`（内含空格，用CSS调整后稍窄且不能被直接复制），同时清理 `.space-around` 元素外侧邻接的空格（`U+0020`）。但在此之前如果 `.space-around` 元素一侧邻接有特定字符或元素时则不在该侧做任何操作：
